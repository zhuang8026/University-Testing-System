...
const POOL = [
  ...
  ...Array(3).fill('sen2zh'),
  ...Array(3).fill('celsyn'),
  ...Array(3).fill('pridesyn'),
  ...Array(3).fill('defn'),
  ...Array(4).fill('fsyn'),
  ...Array(3).fill('notsyn'),
  ...Array(3).fill('sformq'),
  ...Array(5).fill('passage'),
  ...Array(4).fill('olvoc'),
  ...Array(3).fill('oldef'),
];
...
function generateQuiz() {
  ...
  const mandatory = [
    'syn', 'rsyn', 'zh2en', 'en2zh', 'fill', 'wform', 'phrase',
    'sdef', 'szh', 'sen2zh',
    'celsyn', 'pridesyn',
...
<div class="hero">
  ...
  <h1>英文 Unit5 模擬考</h1>
  <p>聚焦 Unit 5 全範圍，每次隨機 ${TOTAL_QS} 題</p>
  <div class="coverage">
    <b>Unit 5 核心單字</b>：14 個單字、同義詞、詞性變化、片語、例句填空<br>
    <b>Unit 5 定義</b>：tradition、ancient 及 6 個運動單字（sailing ～ gymnastics）<br>
    <b>特殊同義詞群</b>：celebrated（7 個同義詞）、pride（6 個同義詞）<br>
    <b>奧運短文閱讀</b>：12 題理解測驗<br>
    <b>奧運生詞</b>：8 個奧運關鍵字彙（honor ～ national）
  </div>
